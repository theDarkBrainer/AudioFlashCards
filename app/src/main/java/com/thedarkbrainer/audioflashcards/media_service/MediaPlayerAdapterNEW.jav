...
public final class MediaPlayerAdapter extends PlayerAdapter {
    ...
    private WordListData mWordListData = null;
    ...
    public MediaPlayerAdapter(Context context, PlaybackInfoListener listener) {
        super(context);
        Log.d("MediaPlayerAdapter", "MediaPlayerAdapter::MediaPlayerAdapter");
    ...
    public void setData(WordListData wordListData, PlayerBox.PlayMode playMode) {
        ...
        mPlayBox = new PlayerBox(mContext, playMode);
    }

    // Implements PlaybackControl.
    @Override
    public void playFromMedia(MediaMetadataCompat metadata) {
        Log.d("MediaPlayerAdapter", "playFromMedia");
    ...
    public MediaMetadataCompat getCurrentMedia() {
        Log.d("MediaPlayerAdapter", "getCurrentMedia");
        return mCurrentMedia;
    }

    private void playFile(String mediaId) {
        Log.d("MediaPlayerAdapter", "playFile: "+mediaId);
        ...
        mMediaId = mediaId;
        WordListData.Data item = mWordListData.getItem(mMediaId);

        mPlayBox.setDataSource(item, new PlayerBox.OnCompletionListener() {
            @Override
    ...
    @Override
    public void onStop() {
        Log.d("MediaPlayerAdapter", "onStop");
    ...
    public boolean isPlaying() {
        return mPlayBox != null && mPlayBox.isPlaying();
    }

    @Override
    protected void onPlay() {
        Log.d("MediaPlayerAdapter", "onPlay");
        if (mPlayBox != null && !mPlayBox.isPlaying()) {
            mPlayBox.play();
    ...
    private void setNewState(@PlaybackStateCompat.State int newPlayerState) {
        Log.d("MediaPlayerAdapter", "setNewState newState="+newPlayerState);
        ...
        if (mSeekWhileNotPlaying >= 0) {
            ...
            reportPosition = mPlayBox == null ? 0 : mPlayBox.getCurrentPosition();
        }
    ...
    public void seekTo(long position) {
        if (mPlayBox != null) {
            if (!mPlayBox.isPlaying()) {
                mSeekWhileNotPlaying = (int) position;
            }
            mPlayBox.seekTo((int) position);
    ...
    public void setVolume(float volume) {
        if (mPlayBox != null) {
            mPlayBox.setVolume(volume, volume);
        }
    }
}
